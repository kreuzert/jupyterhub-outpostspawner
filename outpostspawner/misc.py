...
def _get_shared_semaphore():
    ...
    if _shared_semaphore is None:
        concurrent_limit = int(
            os.environ.get("OUTPOSTSPAWNER_HTTP_CLIENT_CONCURRENT_LIMIT", 10)
        )
...
async def shared_fetch(req, http_client_defaults={}, raise_error=True):
    ...
    async with semaphore:
        return await _get_shared_http_client(http_client_defaults).fetch(
            req, raise_error=raise_error
        )
